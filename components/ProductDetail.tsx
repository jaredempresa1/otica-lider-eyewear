    ...
    <main className="section-shell bg-brand-paper py-8 sm:py-12">
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:gap-16">
        ...
        <div className="flex flex-col justify-center lg:py-8">
          <div className="mt-3 flex items-center gap-3"><p className="font-heading text-2xl font-semibold leading-tight tracking-[-0.03em] text-brand-ink sm:text-3xl">{displayBrand}</p>{brandLogo && <img src={brandLogo} alt={`Logo da marca ${displayBrand}`} className="h-9 w-9 shrink-0 object-contain" />}</div>
          ...
          <div className="mt-6 flex flex-col items-start font-body"><div className="flex items-baseline gap-3 whitespace-nowrap"><span className={`text-[27px] font-semibold ${hasDiscount ? "text-brand-gold" : "text-brand-ink"}`}>{formatBRL(product.price)}</span>{hasDiscount && <span className="text-[15px] text-brand-ink/40 line-through">{formatBRL(product.compare_at_price as number)}</span>}{discountPercent !== null && <span className="text-[13px] font-bold text-red-600">{discountPercent}% OFF</span>}</div>{installmentTotal !== null && product.installments && <span className="mt-2 text-[15px] font-medium leading-6 text-brand-ink"><span className="block">ou até {product.installments.count}x de {formatBRL(product.installments.amount)}</span><span className="block text-[13px] text-brand-ink/65">Total parcelado: {formatBRL(installmentTotal)}</span></span>}</div>
