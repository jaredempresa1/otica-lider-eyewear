    ...
    <main className="section-shell py-8 sm:py-12">
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:gap-16">
        <div>
          ...
          {selectedGallery.length > 0 && <p className="mt-3 font-body text-[10px] font-semibold uppercase tracking-[0.14em] text-brand-ink/40">{selectedColor?.name ? `Galeria da cor ${selectedColor.name}` : "Galeria do produto"} · {selectedGallery.length} {selectedGallery.length === 1 ? "foto" : "fotos"}</p>}
        </div>
        ...
        <div className="flex flex-col justify-center lg:py-8">
          ...
          {product.description && <p className="mt-6 max-w-lg font-body text-sm leading-7 text-brand-ink/65">{product.description}</p>}

          {sortedColors.length > 0 && <div className="mt-8 border-t border-brand-ink/10 pt-6"><div className="flex items-center justify-between gap-3"><p className="font-body text-[10px] font-semibold uppercase tracking-[0.16em] text-brand-ink/55">Cor selecionada</p><span className="font-body text-sm font-medium text-brand-ink">{selectedColor?.name || "Único"}{colorSoldOut ? " · Esgotada" : ""}</span></div><div className="mt-4 flex flex-wrap gap-3">{sortedColors.map((color) => <button key={`${color.name}-${color.hex}`} onClick={() => handleColorSelect(color)} title={color.sold_out ? `${color.name} · Esgotada` : `Ver galeria ${color.name}`} aria-label={color.sold_out ? `${color.name} está esgotada` : `Ver fotos do óculos na cor ${color.name}`} className={`relative h-10 w-10 rounded-full border-2 transition-transform duration-200 hover:scale-105 ${selectedColor?.name === color.name ? "border-brand-gold p-1" : "border-transparent"} ${color.sold_out ? "opacity-50" : ""}`}><span className="relative block h-full w-full overflow-hidden rounded-full border border-brand-ink/10"><span className="absolute inset-0" style={{ backgroundColor: color.hex }} />{color.sold_out && <span className="absolute left-1/2 top-1/2 h-[150%] w-[2px] -translate-x-1/2 -translate-y-1/2 rotate-45 bg-white" />}</span></button>)}</div><p className="mt-3 font-body text-[11px] leading-5 text-brand-ink/45">{colorSoldOut ? "Essa cor está esgotada no momento. Escolha outra opção disponível ou avise que quer ser avisado quando voltar." : "Ao trocar a cor, todas as fotos e ângulos exibidos mudam para essa variação."}</p>{(selectedColor?.frame_color || selectedColor?.lens_color) && <div className="mt-4 grid grid-cols-2 gap-3 rounded-xl bg-brand-paper p-4"><div><p className="font-body text-[9px] font-semibold uppercase tracking-[0.13em] text-brand-ink/45">Armação</p><p className="mt-1 font-body text-sm font-semibold text-brand-ink">{selectedColor.frame_color || "—"}</p></div><div><p className="font-body text-[9px] font-semibold uppercase tracking-[0.13em] text-brand-ink/45">Lentes</p><p className="mt-1 font-body text-sm font-semibold text-brand-ink">{selectedColor.lens_color || "—"}</p></div></div>}</div>}
