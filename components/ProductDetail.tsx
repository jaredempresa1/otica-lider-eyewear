...
function ProductLightbox({ images, initialIndex, alt, onClose }: { images: string[]; initialIndex: number; alt: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex flex-col bg-white p-3 sm:p-6" role="dialog" aria-modal="true" aria-label={`Galeria ampliada de ${alt}`}>
      ...
      <div className="relative min-h-0 flex-1 overflow-hidden rounded-[1.5rem] bg-white">
      ...
      <div className="mt-3 flex gap-2 overflow-x-auto px-1 pb-1 sm:mt-4 sm:justify-center">{images.map((image, index) => <button key={`${image}-${index}`} type="button" onClick={() => selectImage(index)} className={`relative h-16 w-16 shrink-0 overflow-hidden rounded-xl border-2 bg-white transition-colors sm:h-20 sm:w-20 ${currentIndex === index ? "border-brand-gold" : "border-brand-ink/10"}`} aria-label={`Abrir foto ${index + 1}`}><Image src={image} alt={`${alt}, miniatura ${index + 1}`} fill className={index === 0 ? "object-cover" : "object-contain p-1 mix-blend-multiply"} sizes="80px" /></button>)}</div>
    </div>
  );
}
