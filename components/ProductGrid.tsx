...
export default function ProductGrid({ products, emptyMessage, scroll = false, collections, limit }: {
  ...
  useEffect(() => {
    if (!scroll || !shelfRef.current) return;
    const shelf = shelfRef.current;
    const updateProgress = () => {
      const maxScroll = shelf.scrollWidth - shelf.clientWidth;
      setScrollProgress(maxScroll > 0 ? Math.min(1, shelf.scrollLeft / maxScroll) : 1);
    };
    updateProgress();
    shelf.addEventListener("scroll", updateProgress, { passive: true });
    window.addEventListener("resize", updateProgress);
    return () => { shelf.removeEventListener("scroll", updateProgress); window.removeEventListener("resize", updateProgress); };
  }, [scroll, products.length]);
  ...
  if (scroll) {
    ...
    return (
      <>
        <div ref={shelfRef} className="no-scrollbar -mx-5 flex gap-4 overflow-x-auto pb-3 pl-5 pr-5 sm:-mx-8 sm:gap-5 sm:px-8 lg:mx-0 lg:grid lg:grid-cols-3 lg:gap-x-6 lg:gap-y-10 lg:overflow-visible lg:px-0">
