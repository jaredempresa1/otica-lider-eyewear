...
export default function NewsletterSignup() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    const response = await fetch("/api/leads", {
      ...
      body: JSON.stringify({ name: name.trim(), email: normalizedEmail, gender }),
    });
      ...
      <section className="border-t border-brand-logo-orange-dark bg-brand-logo-orange">
        <div className="section-shell py-14 text-center sm:py-20">
          ...
          <p className="mx-auto mt-3 max-w-md font-body text-sm leading-6 text-white/80">
            Em breve você recebe novidades e promoções exclusivas direto no seu e-mail.
          </p>
        </div>
      </section>
    );
    ...
    <section className="border-t border-brand-logo-orange-dark bg-brand-logo-orange">
      <div className="section-shell py-14 text-center sm:py-20">
        ...
        <form onSubmit={handleSubmit} className="mx-auto mt-8 flex max-w-xl flex-col gap-3 text-left">
          ...
          <div className="relative">
            <Mail size={18} strokeWidth={1.8} className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-brand-logo-orange/70" aria-hidden="true" />
            <input
              type="email"
              inputMode="email"
