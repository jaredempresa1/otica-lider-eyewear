  ...
  return (
    <div className="mt-6 border-t border-brand-ink/10 pt-5">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h3 className="font-body text-[14px] font-semibold leading-5 text-brand-ink">Quer que a gente separe esse pedido caso você não finalize agora?</h3>
          <p className="mt-1 font-body text-[12px] leading-5 text-brand-ink/70">Deixe seu WhatsApp e nossa equipe guarda os óculos do seu carrinho.</p>
        </div>
      </div>
      {status === "success" ? (
        <p className="mt-4 flex items-center gap-2 rounded-lg bg-[#e8f6ed] px-3 py-3 font-body text-[13px] text-brand-ink"><Check size={16} className="text-[#079447]" /> {message}</p>
      ) : (
        <form onSubmit={handleSubmit} className="mt-4 flex gap-2">
          ...
          <input type="tel" inputMode="numeric" value={whatsapp} onChange={(event) => setWhatsapp(formatWhatsApp(event.target.value))} placeholder="(83) 99999-9999" aria-label="Seu WhatsApp" className="min-w-0 flex-1 rounded-md border border-brand-ink/15 bg-white px-3 py-2.5 font-body text-[13px] text-brand-ink outline-none placeholder:text-brand-ink/40 focus:border-[#25D366]" />
          <button type="submit" disabled={status === "loading"} className="shrink-0 rounded-md bg-[#25D366] px-3 py-2 font-body text-[10px] font-bold uppercase tracking-[0.08em] text-white transition-colors hover:bg-[#1faa53] disabled:opacity-60">{status === "loading" ? "Salvando" : "Salvar"}</button>
        </form>
      )}
      {status === "error" && <p className="mt-2 font-body text-[12px] text-red-600" role="alert">{message}</p>}
    </div>
  );
}
