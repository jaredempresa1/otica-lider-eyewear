  ...
  return (
    <>
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_410px] lg:gap-12">
        <section>
          ...
          <ul className="divide-y divide-brand-ink/8">
            {items.map((item) => {
              ...
              return (
                <li key={`${item.productId}-${item.colorName}`} onClick={() => router.push(`/produtos/${item.slug}`)} className="group grid cursor-pointer grid-cols-[5rem_minmax(0,1fr)_auto] items-start gap-3 py-5 transition-opacity hover:opacity-80 sm:grid-cols-[6.5rem_minmax(0,1fr)_auto] sm:gap-5">
                  <Link href={`/produtos/${item.slug}`} onClick={(event) => event.stopPropagation()} className="relative h-20 w-20 shrink-0 overflow-hidden rounded-xl bg-brand-sage/40 sm:h-28 sm:w-28" aria-label={`Voltar para ${item.name}`}>
                    {item.image ? (
                  ...
                  <div className="flex min-w-0 flex-col justify-between gap-3 py-0.5">
                    <div>
                      <p className="font-body text-[15px] font-semibold leading-snug text-brand-ink">{item.name}</p>
                      <p className="mt-1 font-body text-[13px] text-brand-ink/50">Cor: {item.colorName}</p>
                      <p className="mt-1 font-body text-[14px] font-semibold text-brand-ink sm:hidden">{formatBRL(itemTotal)}</p>
                    </div>
        ...
        <aside className="h-fit rounded-2xl border border-brand-ink/10 bg-white p-6 shadow-card sm:p-7 lg:sticky lg:top-28">
          <h2 className="font-heading text-2xl font-semibold tracking-[-0.02em] text-brand-ink">Resumo do pedido</h2>

          <div className="mt-6 space-y-3 border-b border-brand-ink/8 pb-6 font-body text-[15px] text-brand-ink">
            ...
            <div className="pt-2">
              ...
              <div className="mt-2 flex gap-2">
                <input id="coupon" value={couponInput} onChange={(event) => setCouponInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") handleApplyCoupon(); }} placeholder="Digite seu cupom" className="min-w-0 flex-1 rounded-xl border border-brand-ink/15 bg-brand-cream/50 px-3 py-2.5 font-body text-[13px] uppercase text-brand-ink outline-none placeholder:text-brand-ink/35 focus:border-brand-gold" />
                <button type="button" onClick={handleApplyCoupon} className="rounded-xl border border-brand-gold px-3 font-body text-[11px] font-semibold uppercase tracking-[0.08em] text-brand-gold transition-colors hover:bg-brand-gold hover:text-white">Aplicar</button>
              </div>
            ...
            <div className="pt-1">
              ...
              <input id="cep" type="text" inputMode="numeric" placeholder="00000-000" value={cep} onChange={(event) => setCep(event.target.value)} className="mt-2 w-full rounded-xl border border-brand-ink/15 bg-brand-cream/50 px-4 py-3 font-body text-[15px] text-brand-ink outline-none placeholder:text-brand-ink/35 focus:border-brand-gold" />
          ...
          <fieldset className="mt-6 border-b border-brand-ink/8 pb-6">
            ...
            {payment.method === "card" && (
              <div className="mt-3 rounded-xl bg-brand-cream/50 p-3">
                ...
                <select id="installments" value={payment.installments} onChange={(event) => handleInstallments(Number(event.target.value))} className="mt-2 w-full rounded-lg border border-brand-ink/15 bg-white px-3 py-2.5 font-body text-[15px] text-brand-ink outline-none focus:border-brand-gold">
		          ...
		          <div className="mt-5 flex items-end justify-between gap-4"><span className="font-body text-[16px] text-brand-ink/70">Total do pedido</span><span className="text-right font-heading text-[26px] font-semibold text-brand-ink">{formatBRL(discountedSubtotal + (shipping?.price || 0))}</span></div>
          ...
          <button onClick={handleCheckout} className="mt-6 flex w-full items-center justify-center gap-2 rounded-full bg-brand-ink px-5 py-4 font-body text-[12px] font-semibold uppercase tracking-[0.15em] text-white transition-all duration-200 hover:bg-brand-gold active:scale-[0.97]">Enviar pedido pelo WhatsApp <span aria-hidden="true">↗</span></button>
          <p className="mt-3 text-center font-body text-[11px] leading-4 text-brand-ink/45">Seu pedido será enviado já organizado, com a forma de pagamento escolhida. A equipe costuma responder em até 1 minuto.</p>
